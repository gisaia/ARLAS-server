...
public class FilteredUpdater extends FluidSearch{
    ...
    public UpdateResponse doAction(UpdateServices.ACTION action, CollectionReference collectionReference, Tag tag, int max_updates) throws IOException, ArlasException {
    ...
    public Script getTagScript(Tag tag, UpdateServices.ACTION action) throws BadRequestException, NotImplementedException {
        String script="";
        if(action.equals(UpdateServices.ACTION.ADD)){
        ...
        if(action.equals(UpdateServices.ACTION.REMOVE)){
            if(tag.value==null){
                throw new io.arlas.server.exceptions.BadRequestException("The tag value must be provided and must not be empty");
            }
            script+="if (ctx._source."+tag.path+" != null) {\n";
        ...
        if(action.equals(UpdateServices.ACTION.REMOVEALL)){
