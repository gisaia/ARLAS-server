...
import io.arlas.server.rest.explore.search.mvt.VectorTileEncoder;
...
public class PBFSearchRESTService extends ExploreRESTServices {

    public PBFSearchRESTService(ExploreServices exploreServices) {
        super(exploreServices);
    }

    @Timed
    @Path("{collection}/_pbfsearch/{z}/{x}/{y}.pbf")
    @GET
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    @Consumes(UTF8JSON)
    @ApiOperation(value = "GeoSearch", produces = MediaType.APPLICATION_OCTET_STREAM, notes = Documentation.GEOSEARCH_OPERATION, consumes = UTF8JSON)
    ...
    public Response geosearch(
        ...
        if (collectionReference.params.centroidPath == null) {
            throw new NotFoundException("centroid not found in "+collection);
        }
        Search search = new Search();
        search.filter = ParamsParser.getFilter(f, q, before, after, pwithin, gwithin, gintersect, notpwithin, notgwithin, notgintersect);
        // TODO Make intersection with pwithin
        BoundingBox bbox = GeoTileUtil.getBoundingBox(new Tile(x,y,z));
        search.filter.pwithin=bbox.getNorth()+","+bbox.getWest()+","+bbox.getSouth()+","+bbox.getEast();

        VectorTileEncoder encoder = new VectorTileEncoder(new Envelope(bbox.getWest(), bbox.getEast(), bbox.getNorth(), bbox.getSouth()));
        ...
        for (int i=0; i<results.length; i++) {
            ...
            Point geoPoint = (Point)GeoTypeMapper.getGeoJsonObject(centroid);
            Coordinate c = new Coordinate(geoPoint.getCoordinates().getLongitude(),geoPoint.getCoordinates().getLatitude());
            encoder.addFeature(collection, null, factory.createPoint(c));
        }
        return cache(Response.ok(encoder.encode()),maxagecache);
    }
}
