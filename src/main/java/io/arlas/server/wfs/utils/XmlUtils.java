...
public class XmlUtils {
    ...
    public static void parsePropertiesXsd(Map<String,CollectionReferenceDescriptionProperty> properties, XMLStreamWriter writer, Stack<String> namespace) throws XMLStreamException {
        for(String key :properties.keySet()){
            CollectionReferenceDescriptionProperty property = properties.get(key);
            namespace.push(key);
            if(property.type== ElasticType.OBJECT){
                parsePropertiesXsd(property.properties,writer,namespace);
            }else{
    ...
    public static void parsePropertiesXml(Map<String,CollectionReferenceDescriptionProperty> properties, XMLStreamWriter writer, Stack<String> namespace,String uri,Object source,String prefix) throws XMLStreamException {
        for(String key :properties.keySet()){
            CollectionReferenceDescriptionProperty property = properties.get(key);
            namespace.push(key);
            if(property.type== ElasticType.OBJECT){
                parsePropertiesXml(property.properties,writer,namespace,uri,source,prefix);
            }else{
