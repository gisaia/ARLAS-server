...
public class ElasticFluidSearch extends FluidSearchService {
    ...
    public Query filterGWithin(String field, String geometry, Boolean righthand) throws ArlasException {
        try {
            JSONObject shapeBuilder = getShapeObject(geometry, righthand);
        ...
        if (aggregationModel.metrics != null) {
            for (Metric m: aggregationModel.metrics) {
                ...
                switch (m.collectFct) {
                    ...
                    case CARDINALITY ->
                            metricAggregation = AggregationBuilders.cardinality().field(m.collectField).build()._toAggregation();
...
/*                Boolean asc = order.equals(Order.asc);
    ...
    // TODO : check the righthand for es8
    ...
    // TODO: Check the righthand stuff with ES 8
