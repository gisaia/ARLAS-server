...
|Topic Category | The topic category is a high-level classification scheme to assist in the grouping and topic-based search of available spatial data resources. Must be one of the values in [this list](http://inspire.ec.europa.eu/metadata-codelist/TopicCategory){:target="_blank"}|[`inspire.topic_categories` node in CollectionReference model](arlas-collection-model.md#inspire)|
...
- `outputSchema` = [http://www.isotc211.org/2005/gmd](http://www.isotc211.org/2005/gmd){:target="_blank"}
- `ElementSetName` = full
